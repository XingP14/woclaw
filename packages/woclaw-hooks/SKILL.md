...
woclaw-hooks 7 shell 钩子配合腾讯 CubeSandbox Rust Wasm 沙箱 (容器级隔离 + Rust 1.81+ Wasm 沙箱) 执行第三方 skill, 解决多 agent 并发场景 hooks 进程级隔离问题 (Gartner 2026 战略趋势「多智能体系统」+「AI 安全平台」并列首日第一线); addyosmani skill.yaml + dotnet/skills + awesome-claude-code 3 层路由; CodexBar macOS 菜单栏监控 (2026-07 AI Agent 技能标准化 + 多智能体沙盒化 + 菜单栏生态 三主战场首日钣定) compatible