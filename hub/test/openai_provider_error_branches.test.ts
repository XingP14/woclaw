/**
 * OpenAIProvider error-branch deeper coverage (07-16 02:23 cron).
 *
 * Companion to `openai_provider.test.ts` (8 happy-path + 500 error cases from
 * 07-13 05:23 cron). This file focuses on the *error / edge / fallback*
 * branches the original file deliberately skipped, mirroring the structure
 * of `ollama_provider_error_branches.test.ts` (07-16 00:43 cron):
 *
 ...
 *      wraps the typed "OpenAI API error 404: <body>". Pins that any
 ...
 *
 *   2. scoreMemory JSON parse failure (raw garbage inside choices[0].message.content):
 *      fetch resolves with `choices[0].message.content = "not json"` -> JSON.parse
 *      throws -> caught by outer try/catch -> success:false, score:5,
 *      reasoning starts with "OpenAI scoring failed:" and contains the
 ...
 *
 *   5. scoreMemory AbortError (openaiChat 30s timeout): fetch rejects with
 *      `Error('OpenAI request aborted after 30000ms timeout')` ->
 *      success:false, reasoning wraps "OpenAI scoring failed:" + the
 ...
 *
 *   7. extractSession HTTP 400 + JSON parse failure combined: HTTP 400 ->
 *      success:false, summary starts with "Extraction failed:" (covers the
 *      success-branch-vs-error-branch symmetry with scoreMemory). Pins that
 *      extractSession shares the same try/catch shape (a regression where
 *      extractSession caught only JSON errors but not HTTP errors would
 *      surface as success:true with `(no summary available)` instead of
 *      an explicit failure).
 *
 *   8. extractSession missing-field defaults: parsed `{}` ->
 ...
 * any time ALLOW). State chain #28 (extending the 07-13 05:23 chain,
 * parallel to ollama_provider_error_branches chain #27).
 */
...
describe('OpenAIProvider error-branch deeper coverage (07-16 02:23 cron)', () => {
  ...
  it('scoreMemory HTTP 404: success:false, score:5, reasoning wraps "OpenAI API error 404: <body>"', async () => {
    fetchMock.mockResolvedValueOnce(chatError(404, 'model gpt-4o-mini does not exist'));
    ...
    expect(r.reasoning).toMatch(/OpenAI API error 404: model gpt-4o-mini does not exist/);
  });
  ...
  it('scoreMemory JSON.parse failure (raw garbage): success:false, score:5, reasoning wraps SyntaxError message', async () => {
    ...
    expect(r.reasoning!.length).toBeGreaterThan('OpenAI scoring failed: '.length + 5);
  });
  ...
  // (parallels openaiChat FETCH_TIMEOUT_MS = 30_000 wiring in extraction_provider_timeout.test.ts)
  it('scoreMemory AbortError (30s timeout): success:false, reasoning wraps "OpenAI request aborted after 30000ms timeout"', async () => {
    fetchMock.mockRejectedValueOnce(new Error('OpenAI request aborted after 30000ms timeout'));
    const p = new OpenAIProvider('test-key');
    const r = await p.scoreMemory('k1', 'hello', []);
    expect(r.success).toBe(false);
    expect(r.score).toBe(5);
    expect(r.reasoning).toMatch(/OpenAI scoring failed: /);
    expect(r.reasoning).toMatch(/OpenAI request aborted after 30000ms timeout/);
  });
  ...
  it('extractSession HTTP 400: success:false, summary starts with "Extraction failed:"', async () => {
    fetchMock.mockResolvedValueOnce(chatError(400, 'bad request: invalid model'));
    const p = new OpenAIProvider('test-key');
    const r = await p.extractSession({ id: 's1', transcript: 'transcript' });
    expect(r.success).toBe(false);
    expect(r.summary).toMatch(/^Extraction failed: /);
    expect(r.summary).toMatch(/OpenAI API error 400: bad request: invalid model/);
  ...
  // "(no summary available)", tags/keyEvents/entities all default to [].
  ...
  // consumers' downstream `.length === 0` checks to throw on undefined.
